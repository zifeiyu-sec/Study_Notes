...
#### 打开yakit，启动靶场，并访问
![[assets/sign数字签名逆向/file-20250714205153750.png]]
![[assets/sign数字签名逆向/file-20250714205255118.png]]
![[assets/sign数字签名逆向/file-20250714205645438.png]]
![[assets/sign数字签名逆向/file-20250714205428086.png]]
![[assets/sign数字签名逆向/file-20250714205508943.png]]
#### 开始测试
随便输入一组弱口令
![[assets/sign数字签名逆向/file-20250714212107807.png]]
![[assets/sign数字签名逆向/file-20250714212115053.png]]
可以看到签名验证成功，用户名密码错误
抓取提交表单内容的数据包，进行爆破

==抓包的时候如果使用bp抓包访问127.0.0.1:8787可能抓取不到包，需要访问前面开启靶场的时候设置的IP==
抓到提交表单的数据包，尝试爆破
![[assets/sign数字签名逆向/file-20250714230449819.png]]
全部失败，因为签名验证失败
![[assets/sign数字签名逆向/file-20250714230539581.png]]
所以当遇到数字签名的时候，不能获取到签名，第一关都过不了，也就说不了用户名和密码了。所以尝试了解数字签名的生成。
#### 签名绕过
还是来到提交表单页面，输入admin,123456,进行测试，打开开发者工具，抓取这个提交表单内容的数据包。
![[assets/sign数字签名逆向/file-20250715143757231.png]]
![[assets/sign数字签名逆向/file-20250715143945040.png]]
...
测试下面数据，
![[assets/sign数字签名逆向/file-20250715144840983.png]]
可以看到签名的值，使用加密工具看自己逆向的是否正确
![[assets/sign数字签名逆向/file-20250715144931739.png]]
可以看到值一样，成功逆向。
...
![[assets/sign数字签名逆向/file-20250715162734629.png]]
当访问到路径/api/questions/lists的时候，说明签名已经产生了，查看==调用堆栈==
堆栈从下到上的执行，查看![[assets/sign数字签名逆向/file-20250715163131455.png]]
查看作用域发现sign已经产生，访问更早一步的匿名，在这个文件中通过正则匹配搜索\bsign来搜索签名前生的位置，![[assets/sign数字签名逆向/file-20250715163342893.png]]
...
>  
通过在网络包中全局搜索通过正则表达式
![[assets/sign数字签名逆向/file-20250715163701579.png]]
点击进去成功定位
![[assets/sign数字签名逆向/file-20250715164223911.png]]
#### 逆向sign
...
>   c = l.$cookies.get("uu"
>通过控制台
>![[assets/sign数字签名逆向/file-20250715165659189.png]]
>如此就可以得到所有的值
>ws()使用md5加密
...
```
将cookie中uu的值暂时替换上去，
![[assets/sign数字签名逆向/file-20250715214320863.png]]
成功逆向sign
...
![[assets/sign数字签名逆向/file-20250715221101386.png]]
...
在这个位置断点调试，通过ai分析
![[assets/sign数字签名逆向/file-20250716091448579.png]]
